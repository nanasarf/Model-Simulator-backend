    ...
    ISimulationModelRegistry models, IClock clock) : IClassroomWorkflow
{
    ...
    public ValueTask RenameTeamAsync(Guid actor, Guid sessionId, Guid teamId, string name, long expectedVersion, string idempotencyKey, CancellationToken ct) => Tx(async () =>
    { if(await Idempotent(actor,"Team.Rename",idempotencyKey,new {sessionId,teamId,name,expectedVersion},ct)) return true; var s=await OwnedSession(actor,sessionId,ct); EnsureDraft(s); EnsureVersion(s,expectedVersion); Required(name,nameof(name)); var t=await db.Teams.SingleOrDefaultAsync(x=>x.Id==teamId&&x.SessionId==sessionId,ct)??throw Error("team.not_found","Team was not found."); t.Name=name.Trim(); s.Version++; Audit(actor,"Team.Renamed","Team",teamId); await SaveIdempotency(actor,"Team.Rename",idempotencyKey,new {sessionId,teamId,name,expectedVersion},null,ct); await db.SaveChangesAsync(ct); return true; },ct).AsVoid();
    public ValueTask DeleteTeamAsync(Guid actor, Guid sessionId, Guid teamId, long expectedVersion, string idempotencyKey, CancellationToken ct) => Tx(async () =>
    { if(await Idempotent(actor,"Team.Delete",idempotencyKey,new {sessionId,teamId,expectedVersion},ct)) return true; var s=await OwnedSession(actor,sessionId,ct); EnsureDraft(s); EnsureVersion(s,expectedVersion); var t=await db.Teams.SingleOrDefaultAsync(x=>x.Id==teamId&&x.SessionId==sessionId,ct)??throw Error("team.not_found","Team was not found."); if(await db.Participants.AnyAsync(x=>x.TeamId==teamId,ct)) throw Error("team.not_empty","Remove team members before deleting the team."); db.Teams.Remove(t); s.Version++; Audit(actor,"Team.Deleted","Team",teamId); await SaveIdempotency(actor,"Team.Delete",idempotencyKey,new {sessionId,teamId,expectedVersion},null,ct); await db.SaveChangesAsync(ct); return true; },ct).AsVoid();
    public ValueTask RemoveTeamMemberAsync(Guid actor, Guid sessionId, Guid teamId, Guid studentId, long expectedVersion, string idempotencyKey, CancellationToken ct) => Tx(async () =>
    { if(await Idempotent(actor,"Participant.Remove",idempotencyKey,new {sessionId,teamId,studentId,expectedVersion},ct)) return true; var s=await OwnedSession(actor,sessionId,ct); EnsureDraft(s); EnsureVersion(s,expectedVersion); var p=await db.Participants.SingleOrDefaultAsync(x=>x.SessionId==sessionId&&x.TeamId==teamId&&x.UserId==studentId,ct)??throw Error("participant.not_found","Participant was not on this team."); p.TeamId=null; p.IsReady=false; p.ReadyAt=null; foreach(var r in await db.RoleAssignments.Where(x=>x.SessionId==sessionId&&x.UserId==studentId&&x.RevokedAt==null).ToListAsync(ct)) r.RevokedAt=clock.UtcNow; s.Version++; Audit(actor,"Participant.Removed","Session",sessionId); await SaveIdempotency(actor,"Participant.Remove",idempotencyKey,new {sessionId,teamId,studentId,expectedVersion},null,ct); await db.SaveChangesAsync(ct); return true; },ct).AsVoid();
    public ValueTask MoveTeamMemberAsync(Guid actor, Guid sessionId, Guid studentId, Guid targetTeamId, long expectedVersion, string idempotencyKey, CancellationToken ct) => Tx(async () =>
    { if(await Idempotent(actor,"Participant.Move",idempotencyKey,new {sessionId,studentId,targetTeamId,expectedVersion},ct)) return true; var s=await OwnedSession(actor,sessionId,ct); EnsureDraft(s); EnsureVersion(s,expectedVersion); if(!await db.Teams.AnyAsync(x=>x.Id==targetTeamId&&x.SessionId==sessionId,ct)) throw Error("team.not_found","Target team was not found."); var p=await db.Participants.SingleOrDefaultAsync(x=>x.SessionId==sessionId&&x.UserId==studentId,ct)??throw Error("participant.not_found","Participant was not found."); p.TeamId=targetTeamId; p.IsReady=false; p.ReadyAt=null; foreach(var r in await db.RoleAssignments.Where(x=>x.SessionId==sessionId&&x.UserId==studentId&&x.RevokedAt==null).ToListAsync(ct)) r.RevokedAt=clock.UtcNow; s.Version++; Audit(actor,"Participant.Moved","Session",sessionId); await SaveIdempotency(actor,"Participant.Move",idempotencyKey,new {sessionId,studentId,targetTeamId,expectedVersion},null,ct); await db.SaveChangesAsync(ct); return true; },ct).AsVoid();
    public ValueTask UnassignRoleAsync(Guid actor, Guid sessionId, Guid assignmentId, long expectedVersion, string idempotencyKey, CancellationToken ct) => Tx(async () =>
    { if(await Idempotent(actor,"Role.Unassign",idempotencyKey,new {sessionId,assignmentId,expectedVersion},ct)) return true; var s=await OwnedSession(actor,sessionId,ct); EnsureDraft(s); EnsureVersion(s,expectedVersion); var r=await db.RoleAssignments.SingleOrDefaultAsync(x=>x.Id==assignmentId&&x.SessionId==sessionId&&x.RevokedAt==null,ct)??throw Error("role_assignment.not_found","Role assignment was not found."); r.RevokedAt=clock.UtcNow; s.Version++; Audit(actor,"Role.Unassigned","Session",sessionId); await SaveIdempotency(actor,"Role.Unassign",idempotencyKey,new {sessionId,assignmentId,expectedVersion},null,ct); await db.SaveChangesAsync(ct); return true; },ct).AsVoid();
    ...
    private static void EnsureVersion(SessionRow session,long expected) { if (session.Version != expected) throw Error("concurrency.conflict","Session setup changed; refresh and review your changes."); }
    private async Task<bool> Idempotent(Guid actor,string operation,string key,object payload,CancellationToken ct)
    { if(string.IsNullOrWhiteSpace(key)) throw Error("idempotency.required","Idempotency-Key is required."); var hash=SHA256.HashData(JsonSerializer.SerializeToUtf8Bytes(payload,JsonOptions)); var prior=await db.IdempotencyRecords.SingleOrDefaultAsync(x=>x.UserId==actor&&x.Operation==operation&&x.Key==key,ct); if(prior is null)return false; if(!CryptographicOperations.FixedTimeEquals(hash,prior.RequestHash)) throw Error("idempotency.conflict","Idempotency key was reused with different content."); return true; }
    private async Task SaveIdempotency(Guid actor,string operation,string key,object payload,Guid? result,CancellationToken ct)
