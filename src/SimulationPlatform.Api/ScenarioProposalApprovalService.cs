    ...
    IProposalCommandFaultInjector faults) : IScenarioProposalApprovalService
{
    public async Task<ScenarioProposalApprovalResult> ApproveAsync(ScenarioProposalApprovalCommand command, CancellationToken ct)
    {
        ...
        // The draft store validates the definition through the shared context. Persist a
        // newly-created definition before it is consumed, while remaining inside the
        // surrounding transaction so rollback still removes it atomically.
