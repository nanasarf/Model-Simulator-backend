...
public sealed class PlatformDbContext(DbContextOptions<PlatformDbContext> options) : DbContext(options)
{
    ...
    public DbSet<ClassroomJoinCodeRow> ClassroomJoinCodes => Set<ClassroomJoinCodeRow>();
    public DbSet<ClassroomJoinRequestRow> ClassroomJoinRequests => Set<ClassroomJoinRequestRow>();
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<ClassroomJoinCodeRow>(e => { e.ToTable("classroom_join_codes", "education"); e.HasKey(x => x.Id); e.HasIndex(x => new { x.NormalizedCode, x.IsActive }).IsUnique(); e.HasOne<ClassroomRow>().WithMany().HasForeignKey(x => x.ClassroomId); });
        b.Entity<ClassroomJoinRequestRow>(e => { e.ToTable("classroom_join_requests", "education"); e.HasKey(x => x.Id); e.HasIndex(x => new { x.ClassroomId, x.StudentUserId, x.Status }); e.Property(x => x.Version).IsConcurrencyToken(); e.HasOne<ClassroomRow>().WithMany().HasForeignKey(x => x.ClassroomId); });
...
public sealed class ClassroomJoinCodeRow { public Guid Id { get; set; } public Guid ClassroomId { get; set; } public required string NormalizedCode { get; set; } public bool IsActive { get; set; } public DateTimeOffset CreatedAt { get; set; } public DateTimeOffset? RevokedAt { get; set; } public Guid CreatedBy { get; set; } }
public sealed class ClassroomJoinRequestRow { public Guid Id { get; set; } public Guid ClassroomId { get; set; } public Guid StudentUserId { get; set; } public required string Status { get; set; } public DateTimeOffset RequestedAt { get; set; } public DateTimeOffset? ReviewedAt { get; set; } public Guid? ReviewedBy { get; set; } public string? DecisionReason { get; set; } public long Version { get; set; } }
